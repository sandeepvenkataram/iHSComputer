...
/*
 * Split code taken from Evan Terna's code at http://stackoverflow.com/questions/236129/splitting-a-string-in-c
 * 
 */
vector<string> &split(const string &s, char delim, vector<string> &elems) {
    stringstream ss(s);
...
void printHelpStatement(void){
	...
	cout<<"arguments: fileListFile numBins"<<endl;
...
void computeStandardizediHS(vector<string> filenames, vector<string> chromosomes){
	...
	for(int i=0; i<freqBins.size(); i++){
		double sum = 0;//std::accumulate(std::begin(freqBins[i]), std::end(freqBins[i]), 0.0);
